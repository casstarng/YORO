...
class FaceDetector(object):
    ...
    def predict(self, frame, thresh=0.85):
        ...
        pred = self.sess.run([self.prob, self.x_center, self.y_center, self.w, self.h], feed_dict={
                             self.training: False, self.img: input_img})
    ...
    def _absolute_bboxes(self, pred, frame, thresh):
        ...
        for j in range(x.shape[0]):
            xc, yc = int((x[j]/YOLO_TARGET) *
                         img_w), int((y[j]/YOLO_TARGET)*img_h)
...
class FaceCorrector(object):
    ...
    def predict(self, frame):
        ...
        input_img = cv2.resize(
            input_img, (CORRECTOR_SIZE, CORRECTOR_SIZE)) / 255.
        input_img = np.reshape(
            input_img, [1, CORRECTOR_SIZE, CORRECTOR_SIZE, 3])

        x, y, w, h = self.sess.run([self.x, self.y, self.w, self.h], feed_dict={
                                   self.training: False, self.img: input_img})
